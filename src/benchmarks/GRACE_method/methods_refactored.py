...
Also this file have some variatons on some functions to garantee the compatibility of GRACE with the rest of the experiments code.
...
def NaiveGradient(x, 
    ...
    if use_cuda:
        x = x.cuda()
    f = net.forward_2_outputs(Variable(x, requires_grad=True)).data.cpu().numpy().flatten()
    I = f.argsort()[::-1]
    fk_hat = I[0]
    fk_i_hat = fk_hat
    ...
    while cont_flag == True:
        ...
        if len(top_feat_idx) == 0:
            ...
            if feature_selector:
                ...
                #print("top_feat_idx -> ", top_feat_idx)
            else:
